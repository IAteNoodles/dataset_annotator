...
if r.status_code == 200:
    result = r.json()
    print(f'Export result: export_id={result.get("export_id")}')
...
if export_id:
    ...
    print(f'Export status: status={r.status_code}, body={r.text[:200]}')
...
if r.status_code == 200:
    result = r.json()
    print(f'Snapshot: snapshot_id={result.get("snapshot_id")}, ann_count={result.get("annotation_count")}')
...
if export_id:
    ...
    if r.status_code == 200:
        result = r.json()
        print(f'Recover: data_items={result.get("data_item_count")}, anns={result.get("annotation_count")}')
